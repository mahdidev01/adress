...
class TranslateCore
{
    ...
    public static function getGenericAdminTranslation($string, &$lang_array, $key = null)
    {
        ...
        if (isset($lang_array['AdminController'.$key]) && !is_array($lang_array['AdminController'.$key])) {
            $str = (string) $lang_array['AdminController'.$key];
        } elseif (isset($lang_array['Helper'.$key]) && !is_array($lang_array['Helper'.$key])) {
            $str = (string) $lang_array['Helper'.$key];
        } elseif (isset($lang_array['AdminTab'.$key]) && !is_array($lang_array['AdminTab'.$key])) {
            $str = (string) $lang_array['AdminTab'.$key];
        } else {
            $str = (string) $string;
        }
        ...
        //     $str = $string;
        // }
        error_log('DEBUG: $str content => ' . print_r($str, true));
